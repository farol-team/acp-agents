...
//!
//! An ACP adapter is often reached through a package runner, so what gets
//! spawned is a wrapper: `npx` → `node` → the agent itself. `kill_on_drop`
//! reaps only the process that was spawned, and the grandchild carries on —
//! reparented to init, holding its ~200 MB, forever. Two of them accumulated
//! in fifteen minutes of ordinary use, and one from a crashed run was still
//! resident three hours later.
//!
//! Two things fix that, and both are needed:
//!
//! * every agent is spawned into **its own process group**, so dropping it
//!   signals the whole wrapper chain rather than the one process a handle is
//!   held to. This covers every ordinary exit;
...
//! adapter-shaped processes: the machine's owner runs `claude` and `codex`
//! themselves, and a reaper that matched on names would kill the terminal
//! session they are sitting in. Only groups this process started are recorded,
//! and a recorded group is killed only if a live process in it still looks
//! like what was recorded — a pid is reused eventually, and the thing wearing
//! it next is not ours to kill.
...
struct Entry {
    ...
    /// start.
    pgid: i32,
    /// What it was, for the identity check at reap time.
    cmd: String,
}
...
fn read(path: &Path) -> Vec<Entry> {
    ...
    let Ok(Value::Array(items)) = serde_json::from_str::<Value>(&text) else {
        warn!(path = %path.display(), "acp: unreadable agent registry, starting over");
        return Vec::new();
    };
    items
        .iter()
...
fn write(path: &Path, entries: &[Entry]) {
    ...
    if let Err(err) = result {
        ...
        warn!(error = %err, path = %path.display(), "acp: could not record the agent group");
    }
}
...
/// wrote down? ANY member counts: the wrapper we spawned may be long dead
...
pub fn reap(path: &Path) -> usize {
    ...
    for entry in entries {
        if !still_ours(entry.pgid, &entry.cmd) {
            debug!(pgid = entry.pgid, "acp: recorded agent is gone already");
            continue;
        }
        ...
        if sent != 0 {
            warn!(pgid = entry.pgid, cmd = %entry.cmd, "acp: could not signal the recorded agent group");
        ...
        if gone {
            info!(pgid = entry.pgid, cmd = %entry.cmd, "acp: killed an agent left by a previous run");
            killed += 1;
        } else {
            warn!(pgid = entry.pgid, cmd = %entry.cmd, "acp: recorded agent ignored SIGTERM; keeping it on the list");
            survivors.push(entry);
        }
    }
    if survivors.is_empty() {
        // The file has served its purpose: everything in it is dead or was not
        // ours. Keeping stale entries would only slow down every launch.
...
/// Signal a whole agent group. Used when the agent is dropped.
#[cfg(unix)]
...
mod tests {
    ...
    fn a_pid_wearing_someone_elses_command_is_not_ours() {
        ...
        let mut child = std::process::Command::new("sleep")
            .arg("30")
            // Its own group, the way real agents are spawned.
            .process_group(0)
            .spawn()
            .expect("spawn sleep");
    ...
    /// The wrapper shape: the leader we spawned is dead, the agent it started
    /// lives on in the same group. The reaper must still take it down.
    #[cfg(unix)]
    #[test]
    ...
    /// A group that shrugs off SIGTERM must not be forgotten.
    #[cfg(unix)]
    #[test]
